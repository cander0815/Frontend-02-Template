...
function addCssRules(c) {
  const ast = css.parse(c);
...
function computeCss(el) {
  ...
  for (let rule of rules) {
    ...
    if (matched) {
      ...
      for (let v of rule.declarations) {
        if (!computedStyle[v.property]) {
          computedStyle[v.property] = {};
        }
        if (!computedStyle[v.property].specificity) {
          computedStyle[v.property].specificity = sp;
          computedStyle[v.property].value = v.value;
        } else if (compare(computedStyle[v.property].specificity, sp) < 0) {
          computedStyle[v.property].specificity = sp;
          computedStyle[v.property].value = v.value;
        }
      }
